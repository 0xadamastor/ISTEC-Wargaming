...
class ToolsOutputParser:
    ...
    def parse_tool_output(self, tool_name, output, target=None):
        """Parse o output da ferramenta e cria nodes/connections"""
        if tool_name in self.parsers:
            return self.parsers[tool_name](output, target)
        return []
    
    def parse_nmap_output(self, output, target=None):
        """Parse output do Nmap e cria nodes de hosts, serviços e portas"""
        nodes_created = []
        
        # Padrões para extrair informações do Nmap
    ...
    def parse_whois_output(self, output, target=None):
        """Parse output do Whois e cria nodes de domínio"""
        nodes_created = []
        ...
        # Padrões comuns de output WHOIS
        ...
        for i, nameserver in enumerate(domain_data['name_servers']):
            ...
            # Conectar domínio ao nameserver
    ...
    def parse_nslookup_output(self, output, target=None):
        ...
        # Padrões para diferentes tipos de registros DNS
        ...
        # Criar node de domínio principal
        if target:
            domain_position = QPointF(0, 0)
            domain_node = self.graph_manager.add_node('domain', domain_position, domain_data)
            nodes_created.append(domain_node)
            
            # Conectar domínio aos IPs
    ...
    def parse_sublist3r_output(self, output, target=None):
        """Parse output do Sublist3r e cria subdomínios"""
        nodes_created = []
        
        if not target:
            return nodes_created
        
        # Padrão para subdomínios (linhas que contêm o domínio alvo)
        ...
        # Criar nodes para subdomínios
        for i, subdomain in enumerate(set(subdomains)):  # Remove duplicatas
            subdomain_data = {
                'name': subdomain,
            ...
            # Conectar domínio principal ao subdomínio
    ...
    def parse_exiftool_output(self, output, target=None):
        """Parse output do ExifTool e cria node de screenshot com metadados"""
        nodes_created = []
        ...
        # Extrair informações específicas úteis
        screenshot_data = {
            'filename': target,
            'file_path': target,
            'metadata': metadata,
        ...
        # Adicionar campos específicos se disponíveis
    ...
    def parse_gobuster_output(self, output, target=None):
        """Parse output do Gobuster e cria nodes de diretórios/arquivos"""
        nodes_created = []
        
        if not target:
            return nodes_created
        
        # Padrões para resultados do Gobuster
        ...
        # Extrair diretórios/arquivos encontrados
        directory_matches = re.findall(directory_pattern, output)
        for path, status in directory_matches:
            endpoint_data = {
                'path': path,
                'method': 'GET',  # Assumindo GET para descoberta
                'status_code': int(status),
                'authentication_required': False,
                'vulnerable': int(status) in [200, 301, 302, 307],  # Status codes interessantes
            ...
            # Conectar aplicação web ao endpoint
