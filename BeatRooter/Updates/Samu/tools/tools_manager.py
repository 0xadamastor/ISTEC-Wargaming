    ...
    def run(self):
        try:
            ...
            if platform.system() == "Windows":
                wsl_tools = ['gobuster', 'sublist3r', 'whois', 'dig']
                if any(tool in self.command[0].lower() for tool in wsl_tools):
                    if WSLHandler.is_wsl_available():
                        use_wsl = True
...
class ToolsManager(QWidget):
    ...
    def setup_ui(self):
        ...
        # Output Section - Altura fixa para evitar scroll externo
    ...
    def create_tool_button(self, name, description):
        ...
        btn.setMinimumHeight(45)  # Mais compacto
        btn.setStyleSheet(f"""
            QPushButton {{
                background-color: #2d3748;
                color: #e2e8f0;
        ...
        # Layout personalizado para o botão
    ...
    def get_command_for_tool(self, tool_name, target, options):
        base_commands = {
            'exiftool': ['exiftool', target] + (options.split() if options else []),
            'gobuster': ['gobuster', 'dir', '-u', target, '-w', '/usr/share/dirb/wordlists/common.txt'] + (options.split() if options else []),
            'nmap': ['nmap'] + (options.split() if options else ['-sV', '-sC']) + [target],
            'nslookup': ['nslookup', target] + (options.split() if options else []),
            'sublist3r': ['sublist3r', '-d', target] + (options.split() if options else []),
            'whois': ['whois', target] + (options.split() if options else [])
        }
    ...
    def execute_command(self, command_parts):
        ...
        self.current_command_thread = CommandThread(command_parts)
    ...
    def on_command_finished(self, exit_code):
        ...
        # Processar output automaticamente se o comando foi bem-sucedido
        if exit_code == 0 and hasattr(self, 'current_tool'):
            output_text = self.output_text.toPlainText()
            target = self.target_input.text().strip()
            
            try:
                # Converter output em nodes
