...
class DynamicEdge(QGraphicsPathItem):
    def __init__(self, source_node, target_node, edge_data=None):
        ...
        # ADICIONE ESTA LINHA para aceitar eventos de contexto
    ...
    def update_label(self):
        if self.edge_data:
            ...
            if not label or label == "Connected":
                label = f"{self.edge_data.edge_type}"
    ...
    def edit_edge(self):
        ...
        label_input = QLineEdit(self.edge_data.label)
    ...
    def delete_edge(self):
        if self.edge_data:
            # Encontrar a main window e chamar o método de exclusão
    ...
    def debug_context_menu(self):
        ...
        # Verifica se consegue encontrar a main window
        found = False
    ...
    def call_main_window_method(self, method_name, *args):
        ...
        # Tenta encontrar a main window de várias formas
        main_window = None
        
        # Método 1: Procura nos widgets top-level
        for widget in QApplication.topLevelWidgets():
            if hasattr(widget, method_name):
                main_window = widget
                break
        
        # Método 2: Se não encontrou, tenta via parent chain
        if not main_window:
            parent = self.parent()
            while parent:
    ...
    def mouseDoubleClickEvent(self, event):
        """Clique duplo - mostra menu de contexto em vez de editar diretamente"""
        if event.button() == Qt.MouseButton.LeftButton:
            # Em vez de chamar edit_edge diretamente, mostra o menu de contexto
            self.show_context_menu(event.screenPos())
        else:
            super().mouseDoubleClickEvent(event)

    def show_context_menu(self, screen_pos):
        """Mostra o menu de contexto numa posição específica"""
        menu = QMenu()
        menu.setStyleSheet("""
            QMenu {
                background-color: #1a1f2c;
        ...
        # CORREÇÃO: Criar QAction com a sintaxe correta
        edit_action = QAction("✎ EDIT EDGE", menu)  # Parent é o menu, não self
        edit_action.triggered.connect(self.edit_edge)
        menu.addAction(edit_action)
        
        # CORREÇÃO: Criar QAction com a sintaxe correta  
        delete_action = QAction("🗑 DELETE EDGE", menu)  # Parent é o menu, não self
        delete_action.triggered.connect(self.delete_edge)
        menu.addAction(delete_action)
        
        # Mostra o menu na posição fornecida
        menu.exec(screen_pos)

    def contextMenuEvent(self, event):
        """Menu de contexto quando clica com botão direito na conexão"""
        self.show_context_menu(event.screenPos())
        event.accept()

    def mousePressEvent(self, event):
        """Handle mouse press events - para capturar clique direito"""
        if event.button() == Qt.MouseButton.RightButton:
            # Marca a edge como selecionada
            self.setSelected(True)
            # Mostra o menu de contexto imediatamente
            self.show_context_menu(event.screenPos())
            event.accept()  # Marca o evento como tratado
        else:
