...
class DigitalDetectiveBoard(QMainWindow):
    def __init__(self, project_type=None, category=None, template_data=None):
        ...
        self.settings = QSettings('BeatRooter', 'BeatRooter')
    ...
    def create_connection(self, source_id, target_id):
        try:
            ...
            edge = self.graph_manager.connect_nodes(source_id, target_id, "Connected")
    ...
    def closeEvent(self, event):
        ...
        reply = QMessageBox.question(
            self,
            'Exit BeatRooter',
            'Are you sure you want to exit?\n\nMake sure to save your project before closing.',
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def load_graph_data(self, graph_data):
        ...
        # Restore project type and category from metadata if available
        if 'project_type' in graph_data.metadata:
            self.project_type = graph_data.metadata['project_type']
        if 'category' in graph_data.metadata:
            self.category = graph_data.metadata['category']
            # Update the toolbox with the loaded category
            if self.category:
                self.toolbox.update_category(self.category)
        
        # Update window title with loaded project info
        if self.storage_manager.current_file:
            import os
            project_name = os.path.basename(self.storage_manager.current_file)
            self.setWindowTitle(f"{self.get_project_title()} - {project_name}")
        else:
            self.setWindowTitle(self.get_project_title())
        
        for node in graph_data.nodes.values():
            self.graph_manager.graph_data.nodes[node.id] = node
            node_widget = NodeWidget(node, category=self.category)
            self.canvas_widget.add_node_widget(node_widget)
            node_widget.node_updated.connect(self.on_node_selected)
            node_widget.connection_started.connect(self.start_connection)
            self.node_widgets[node.id] = node_widget
            # Force update the node widget appearance
            node_widget.update()
    ...
    def open_investigation(self):
        ...
        if filename:
            try:
                ...
                self.settings.setValue('last_project', filename)
    ...
    def save_investigation_as(self):
        ...
        if filename:
            ...
            # Save project type and category to metadata
            if self.project_type:
                self.graph_manager.graph_data.metadata['project_type'] = self.project_type
            if self.category:
                self.graph_manager.graph_data.metadata['category'] = self.category
    ...
    def show_about(self):
        ...
        <li>BeatHelper - Manual finder tool</li>
        </ul>
        <p>© 2024 BeatRooter</p>
        """
