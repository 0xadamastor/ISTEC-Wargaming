...
class NodeWidget(QGraphicsObject):
    ...
    def get_node_accent_color(self):
        # USA A CATEGORIA se disponível
        color_hex = NodeFactory.get_node_color(self.node.type, self.category)
        print(f"DEBUG: Node type '{self.node.type}', category '{self.category}' -> Color: '{color_hex}'")  # DEBUG
        return QColor(color_hex)
    
    def get_node_display_name(self):
        """Obtém o nome de exibição do node usando o NodeFactory"""
        # CORREÇÃO: Passar a categoria para o NodeFactory
        name = NodeFactory.get_node_name(self.node.type, self.category)
        return name
    
    def get_node_symbol(self):
        # Mapeamento completo de símbolos para todos os tipos de nodes
        symbols = {
            # Nodes base originais
            'user': "[USER]",
            'ip': "[IP]",
            'domain': "[DOMAIN]",
            'credential': "[PASS]",
    ...
    def paint(self, painter, option, widget):
        ...
        base_color = QColor(NodeFactory.get_node_color(self.node.type))
    ...
    def draw_terminal_header(self, painter, accent_color):
        ...
        # USA O NOME DO NODE FACTORY em vez do tipo bruto
    ...
    def get_all_possible_fields(self):
        ...
        if not lines:
            lines.append(self.get_node_display_name().upper())
