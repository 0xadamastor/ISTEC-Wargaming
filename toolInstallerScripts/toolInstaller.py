...
def remove_all_tools():
    ...
    confirm = input(f"{RED}Tem certeza que deseja remover TODAS as ferramentas? [s/N]: {RESET}").lower()
...
def main_menu():
    ...
    if not SCRIPTS_DIR.exists():
        print(f"{RED}[✗] Diretório 'installers' não encontrado!{RESET}")
        print(f"{YELLOW}Crie o diretório e coloque os scripts .sh lá dentro.{RESET}")
        sys.exit(1)
    
    while True:
        clear_screen()
        print_header("GESTOR DE FERRAMENTAS DE SEGURANÇA")
